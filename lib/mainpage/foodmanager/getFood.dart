      ...
      if (!snapshot.hasData)
        ...
        if (commentCount0 > 0) {
          ...
          return seven_food_pic(
            title: commentsData0
                .map((comment) => comment['title'] as String)
                .toList(),
            date: commentsData0
            ...
            press: () {},
            image: commentsData0
                .map((comment) => comment['image'] as String)
                .toList(),
            color: kFoodexpired,
          );
        } else {
  ...
  return StreamBuilder<QuerySnapshot>(
    ...
    builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> snapshot) {
      if (!snapshot.hasData)
        ...
        // 將留言資料保存到 commentsData 中
        commentsData7 = snapshot.data!.docs
            .map((document) {
              // DateTime foodDate = DateTime.parse(document['date']);
              Timestamp timestamp = document['EXP'] as Timestamp;
              DateTime expDate = timestamp.toDate();

              if (expDate.isAfter(oneDaysLater) == true &&
                  expDate.isBefore(sevenDaysLater) == true) {
                // print("有抓到");
                String formattedDate = DateFormat('yyyy-MM-dd').format(expDate);
                return {
                  'title': document['food_name'] as String,
...
StreamBuilder<QuerySnapshot> getFood15() {
  return StreamBuilder<QuerySnapshot>(
    ...
    builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> snapshot) {
      ...
      // 將留言資料保存到 commentsData 中
      commentsData15 = snapshot.data!.docs
          .map((document) {
            // DateTime foodDate = DateTime.parse(document['date']);
            Timestamp timestamp = document['EXP'] as Timestamp;
            DateTime expDate = timestamp.toDate();
            ...
            if (expDate.isAfter(eightDaysLater) == true &&
                expDate.isBefore(fifteenDaysLater) == true) {
              // print("有抓到");
              String formattedDate = DateFormat('yyyy-MM-dd').format(expDate);
              return {
                'title': document['food_name'] as String,
...
StreamBuilder<QuerySnapshot> getFood30() {
  return StreamBuilder<QuerySnapshot>(
    ...
    builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> snapshot) {
      ...
      // 將留言資料保存到 commentsData 中
      commentsData30 = snapshot.data!.docs
          .map((document) {
            // DateTime foodDate = DateTime.parse(document['date']);
            Timestamp timestamp = document['EXP'] as Timestamp;
            DateTime expDate = timestamp.toDate();
            ...
            // for (var comment in commentsData7){
            //   bool isInSevenDays = expDate.isBefore(sevenDaysLater);
            if (expDate.isAfter(sixteenDaysLater) == true &&
                expDate.isBefore(thirtyDaysLater) == true) {
              // print("有抓到");
              String formattedDate = DateFormat('yyyy-MM-dd').format(expDate);
              return {
                'title': document['food_name'] as String,
...
StreamBuilder<QuerySnapshot> getFood31() {
  return StreamBuilder<QuerySnapshot>(
    ...
    builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> snapshot) {
      ...
      // 將留言資料保存到 commentsData 中
      commentsData31 = snapshot.data!.docs
          .map((document) {
            // DateTime foodDate = DateTime.parse(document['date']);
            Timestamp timestamp = document['EXP'] as Timestamp;
            DateTime expDate = timestamp.toDate();
